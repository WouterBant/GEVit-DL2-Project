...
GEViT is an E(2) equivariant vision model which builds upon GSA-nets by altering the positional encoding. Although GEViT is more expressive than G-CNNs, it is a very computationally expensive model and does not have superior performance over G-CNNs. In our experiments we found that we get exceptional results with a normal vision transformer, which we augment to be equivariant post-hoc, this is also significantly less computationally demanding. Lastly, we utilize a G-CNN to extract equivariant features and downsize the image before passing it to the GEViT. This drastically improves efficiency, getting similar results in %5 of the epochs with 40 times as fast model. 
